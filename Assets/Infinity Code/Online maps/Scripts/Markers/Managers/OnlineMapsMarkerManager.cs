...
public class OnlineMapsMarkerManager : OnlineMapsMarkerManagerBase<OnlineMapsMarkerManager, OnlineMapsMarker>
{
    ...
    public static OnlineMapsMarker CreateUserItem(double longitude, double latitude, Texture2D texture = null, string label = "")
    {
        if (instance != null) return instance.CreateTarget(longitude, latitude, texture, label);
        return null;
    }

    public OnlineMapsMarker CreateTarget(double longitude, double latitude, Texture2D texture = null, string label = "")
    {
        if (texture == null) texture = defaultTexture;
        OnlineMapsMarker marker = _CreateItem(longitude, latitude);
        marker.manager = this;
        marker.texture = texture; // added this
        marker.label = label;
        marker.align = defaultAlign;
        marker.scale = defaultScale;
        marker.range = new OnlineMapsRange(7, 20);// and this
        marker.Init();
        Redraw();
        return marker;
    }
