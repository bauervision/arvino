...
public class SURGE_GPS : MonoBehaviour
{


    public static SURGE_GPS Instance { set; get; }
    ...
    private void Start()
    {
        ...
        if (Application.isEditor)
        {
            ...
            else
            {
                ...
                if (gpsMode == SURGEgpsMode.Unity3D)
                {
        ...
        if (!Application.isEditor)
        {
            if (GPS)
            {
                ...
                else // if we have valid location services, run everything else
                {
                    ...
                    // make sure we have GPS data first
                    if (FirebaseInit.initialized)
                    {
                        // if there is a need to push an update...
                        if ((androidUser._Lat != _UserLat) || (androidUser._Lon != _UserLon))
                            FirebaseInit.SendMyGPSCoords(androidUser);
