...
public class SURGE_Target : MonoBehaviour
{
    public static SURGE_Target instance;
    ...
    public SURGE_Target_Type _Type;
    public string _ID;
    ...
    IEnumerator PlaySound()
    {
        while (_isListening)
        {
            AudioClip soundClipType = GetClipType();
            myAudioSource.PlayOneShot(soundClipType);
            yield return new WaitForSeconds(soundClipType.length);

            AudioClip soundClipRange = GetRange();
            myAudioSource.PlayOneShot(soundClipRange);

            // finally check to see if this target is behind the user
            if (isBehindYou)
            {
                yield return new WaitForSeconds(soundClipRange.length);
                AudioClip soundClipBehind = AudioManager.instance.baseAudio.BehindYou;
                // alway make sure to get an updated clip as distance can change
                myAudioSource.PlayOneShot(soundClipBehind);
            }

            yield return new WaitForSeconds(UIManager.instance.audioInterval);
        }
    ...
    private Vector3 CalculateCameraPosUpdate()
    {
        ...
        double clat = _Lat - SURGE_GPS.Instance._UserLat;
        clat = GetMetricCoordinates(earthRadius, clat);
        double clon = _Lon - SURGE_GPS.Instance._UserLon;
    ...
    private void HandleScreenPos()
    {
        ...
        {
            isBehindYou = false;
            // bounding box tracker displays
            if (!UIManager.instance.displayFriendlies)// if we want to hide friendlies
                myTracker.SetActive((_Type != SURGE_Target_Type.ANDROID && _Type != SURGE_Target_Type.DRONE));
            else
    ...
    private void HandleTargetRingIndicatorUpdate()
    {
        ...
        // if we want to hide friendlies
        if (!UIManager.instance.displayFriendlies)
        {
            myIndicator.SetActive((_Type != SURGE_Target_Type.ANDROID && _Type != SURGE_Target_Type.DRONE));

        }
        else if (UIManager.instance.showTargetRing && !myIndicator.activeInHierarchy)     // make sure we only turn it on, when it is not on, but the user wants it on
        ...
        if (!Application.isEditor)
            ...
            finalAngle = angle - SURGE_GPS.compassAngle;
    ...
    private void HandleDistanceToCamera()
    {
        ...
        distanceToCamera = SURGE_Utils.DistanceBetweenPoints(myCoords, SURGE_GPS.Instance._UserCoords).magnitude;
    ...
    /// </summary>
    public void UpdatePosition(SurgeActor actor, SURGE_Target_Type type)
    {
        if (myTracker != null)
            myTracker.GetComponent<Image>().enabled = !UIManager.instance.displayGeometry;
    ...
    public void SetInitialPosition(SurgeActor actor, SURGE_Target_Type type)
    {
    ...
    public void SetActiveListeningTarget() { _isListening = true; runningCoroutine = StartCoroutine(PlaySound()); }

    public void StopListeningTarget() { _isListening = false; StopCoroutine(runningCoroutine); }

    private AudioClip GetClipType()
    {
        switch (_Type)
        {
            case SURGE_Target_Type.OBJECTIVE: return AudioManager.instance.baseAudio.Objective;

            case SURGE_Target_Type.PERSON: return AudioManager.instance.baseAudio.Person;

            default: return AudioManager.instance.baseAudio.Vehicle;
        }
    }
    private AudioClip GetRange()
    {
        if (distanceToCamera > 1)
            return AudioManager.instance.targetRanges.More_Than_1K;
        else if (distanceToCamera < 1 && distanceToCamera > 0.5f)
            return AudioManager.instance.targetRanges.Less_Than_1KM;
        else if (distanceToCamera < 0.5f && distanceToCamera > 0.05f)
            return AudioManager.instance.targetRanges.Less_Than_500M;
        else return AudioManager.instance.targetRanges.Less_Than_50M;
    }
